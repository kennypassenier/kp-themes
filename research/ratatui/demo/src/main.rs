...
fn main() -> io::Result<()> {
    ...
    let fps: u32 = arg("--fps").and_then(|v| v.parse().ok()).unwrap_or(15).clamp(1, 60);
    let frame_every = Duration::from_micros(1_000_000 / fps as u64);
    let exit_after = arg("--exit-after").and_then(|v| v.parse::<f64>().ok()).map(Duration::from_secs_f64);

    let mut app = App::new(cfg, depth, path);
    app.screen = if arg("--screen").as_deref() == Some("components") { Screen::Components } else { Screen::Dashboard };
    app.dash.fps = fps;
    ...
    let result = loop {
        ...
        if now >= next_sample {
            if let Some(s) = sampler.sample() {
                app.dash.push_sample(now.duration_since(start).as_secs_f64(), s);
            }
        ...
        if now >= next_frame {
            #[cfg(feature = "tachyonfx")]
            {
                ...
                let wanted = app.screen == Screen::Components && app.config.motion == Motion::Full;
                let stale = tfx.as_ref().map(|(_, k)| *k != key).unwrap_or(wanted) || app.reveal_ms == 0;
                if stale {
                    tfx = wanted.then(|| {
        ...
        let wait = next_frame.min(next_sample).saturating_duration_since(Instant::now());
        if event::poll(wait)?
    ...
    if exit_after.is_some() {
        ...
        let ticks = |p: &str| fs::read_to_string(p).map(|t| live::parse_proc_ticks(&t)).unwrap_or(0) as f64;
        let own = ticks("/proc/self/stat") / live::CLK_TCK;
        let child = feed.child_pid().map(|p| ticks(&format!("/proc/{p}/stat")) / live::CLK_TCK).unwrap_or(0.0);
        eprintln!(

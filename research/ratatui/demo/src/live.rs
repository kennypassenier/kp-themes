...
pub fn parse_stat(text: &str) -> Vec<CpuTimes> {
    text.lines()
        ...
        .map(|l| {
            let n: Vec<u64> = l.split_whitespace().skip(1).filter_map(|v| v.parse().ok()).collect();
...
pub fn parse_net_dev(text: &str) -> (u64, u64) {
    text.lines()
        ...
        .map(|(_, rest)| {
            let n: Vec<u64> = rest.split_whitespace().filter_map(|v| v.parse().ok()).collect();
            (n.first().copied().unwrap_or(0), n.get(8).copied().unwrap_or(0))
        })
...
pub fn rates(prev: &Counters, cur: &Counters, secs: f64) -> Sample {
    ...
    let busy = |a: CpuTimes, b: CpuTimes| {
        ...
        if dt == 0 { 0.0 } else { 100.0 * (dt - b.idle.saturating_sub(a.idle).min(dt)) as f64 / dt as f64 }
    };
    let pcts: Vec<f64> = prev.cpu.iter().zip(&cur.cpu).map(|(a, b)| busy(*a, *b)).collect();
    ...
    Sample {
        ...
        mem_used_pct: if cur.mem_total_kib == 0 { 0.0 } else { 100.0 * used as f64 / cur.mem_total_kib as f64 },
...
impl Sampler {
    pub fn new(child: Option<u32>) -> Self {
        Sampler { proc: PathBuf::from("/proc"), child, last: None }
    }
    ...
    fn read(&self) -> io::Result<Counters> {
        ...
        let (net_rx_bytes, net_tx_bytes) = get("net/dev").map(|t| parse_net_dev(&t)).unwrap_or_default();
        // A physical block device has a `device` link; zram, loop and dm do not.
        let (disk_read_bytes, disk_write_bytes) = get("diskstats")
            .map(|t| parse_diskstats(&t, |name| Path::new("/sys/block").join(name).join("device").exists()))
            .unwrap_or_default();
        let ticks = |pid: &str| get(&format!("{pid}/stat")).map(|t| parse_proc_ticks(&t)).unwrap_or(0);
        Ok(Counters {
            cpu: parse_stat(&get("stat")?),
            mem_total_kib,
            mem_available_kib,
            load: get("loadavg").map(|t| parse_loadavg(&t)).unwrap_or_default(),
            net_rx_bytes,
            net_tx_bytes,
            disk_read_bytes,
            disk_write_bytes,
    ...
    pub fn sample(&mut self) -> Option<Sample> {
        ...
        let out = self.last.as_ref().map(|(at, prev)| rates(prev, &cur, now.duration_since(*at).as_secs_f64()));

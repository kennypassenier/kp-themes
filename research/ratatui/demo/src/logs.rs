...
impl LogLine {
    pub fn new(time: &str, host: &str, unit: &str, severity: Severity, message: &str) -> Self {
        LogLine { seq: 0, time: time.into(), host: host.into(), unit: unit.into(), severity, message: message.into() }
    }
}
...
pub fn clock(micros: u64, offset_secs: i64) -> String {
    ...
    format!("{:02}:{:02}:{:02}.{:03}", day / 3600, day / 60 % 60, day % 60, micros / 1000 % 1000)
}

/// The machine's UTC offset, asked once of `date +%z` (`+0200`). The demo
/// has no time-zone crate; 0 when `date` is missing.
pub fn local_offset_secs() -> i64 {
    let Ok(out) = Command::new("date").arg("+%z").stderr(Stdio::null()).output() else { return 0 };
...
pub fn parse_journal_json(line: &str, offset_secs: i64) -> Option<LogLine> {
    ...
    let text = |k: &str| -> Option<String> {
        match &v[k] {
            ...
            serde_json::Value::Array(a) => {
                let bytes: Vec<u8> = a.iter().filter_map(|b| b.as_u64().map(|b| b as u8)).collect();
    ...
    let severity = Severity::from_priority(text("PRIORITY").and_then(|p| p.parse().ok()).unwrap_or(6));
    let ident = text("SYSLOG_IDENTIFIER").or_else(|| text("_COMM")).unwrap_or_else(|| "?".into());
...
impl LogBuffer {
    pub fn new(cap: usize) -> Self {
        LogBuffer { lines: VecDeque::new(), cap, next_seq: 1, pin: None, scroll: 0, filter: Severity::Debug }
    }
    ...
    pub fn toggle_pause(&mut self) {
        self.pin = if self.pin.is_some() { None } else { Some(self.last_seq()) };
        self.scroll = 0;
    }
    ...
    fn shown(&self) -> impl DoubleEndedIterator<Item = &LogLine> {
        ...
        self.lines.iter().filter(move |l| l.severity <= self.filter && l.seq <= pin)
    }
...
pub enum Feed {
    Journal { child: Child, rx: Receiver<LogLine>, got_any: bool, started: Instant },
    Synthetic { next_at: Instant, state: u64, offset: i64, reason: &'static str },
}
...
impl Feed {
    ...
    pub fn journal() -> std::io::Result<Feed> {
        ...
        Ok(Feed::Journal { child, rx, got_any: false, started: Instant::now() })
    }

    pub fn synthetic(reason: &'static str) -> Feed {
        Feed::Synthetic { next_at: Instant::now(), state: 0x2545_F491_4F6C_DD1D, offset: local_offset_secs(), reason }
    }
    ...
    pub fn drain(&mut self, buf: &mut LogBuffer, max: usize) {
        ...
        match self {
            Feed::Journal { rx, got_any, started, .. } => {
                for _ in 0..max {
                    match rx.try_recv() {
                        Ok(line) => {
                            *got_any = true;
            ...
            Feed::Synthetic { next_at, state, offset, .. } => {
                let now = Instant::now();
                let mut n = 0;
...
const SYNTHETIC: &[(Severity, &str, &str)] = &[
    (Severity::Info, "restic[2231]", "snapshot 4f1c9a02 saved, 1,284 files changed, 212 MiB added"),
    (Severity::Info, "nginx[918]", "10.10.10.6 GET /api/health 200 3 ms"),
    (Severity::Debug, "dockerd[1102]", "container web-2 health check passed in 41 ms"),
    (Severity::Notice, "systemd[1]", "Started backup.service - nightly off-site backup."),
    (Severity::Warning, "smartd[744]", "Device /dev/nvme0n1, temperature 71 C reached the warning limit"),
    (Severity::Info, "sshd[40117]", "Accepted publickey for kenny from 10.10.10.3 port 51522"),
    (Severity::Error, "caddy[1310]", "certificate renewal for media.home failed: DNS challenge timed out"),
    (Severity::Debug, "kernel", "eno1: link speed 1000 Mbps, full duplex"),
    (Severity::Notice, "systemd[1]", "Finished logrotate.service - rotated 14 log files."),
    (Severity::Critical, "kernel", "EXT4-fs error (device sdb1): inode 1318 has a bad checksum"),
    (Severity::Info, "jellyfin[2890]", "Playback started: episode 4 on the living-room client"),
    (Severity::Warning, "dockerd[1102]", "container worker-1 restarted 3 times in 10 minutes"),
];

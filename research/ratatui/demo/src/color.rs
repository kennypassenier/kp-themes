...
/// An 8-bit sRGB colour as the web build resolves it.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct Rgb(pub u8, pub u8, pub u8);

/// What a token is used as. Decides the 16-colour fallback, where a nearest
/// match by distance alone would paint a light theme's paper white on a
/// dark terminal, or turn every dark surface into the same black.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum Role {
    /// A ground: page, card, popover.
    Surface,
    /// Text on a ground.
    Ink,
    /// Secondary text.
    MutedInk,
    /// A coloured plate an action sits on (primary, destructive).
    Fill,
    /// Text on a fill.
    OnFill,
    /// A border.
    Line,
    /// A colour that means something (ring, success, signal).
    Signal,
}
...
impl ColorDepth {
    ...
    pub fn detect(flag: Option<&str>, env: impl Fn(&str) -> Option<String>) -> Self {
        if let Some(d) = flag.map(str::to_string).or_else(|| env("KP_COLORS")).and_then(|v| Self::parse(&v)) {
            return d;
        }
...
fn dist(a: Rgb, b: Rgb) -> i64 {
    ...
    let (dr, dg, db) = (a.0 as i64 - b.0 as i64, a.1 as i64 - b.1 as i64, a.2 as i64 - b.2 as i64);

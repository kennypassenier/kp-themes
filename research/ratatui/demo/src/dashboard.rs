...
use ratatui::{
    ...
    symbols::Marker,
    text::{Line, Span},
    widgets::{Axis, Bar, BarChart, BarGroup, Block, Chart, Dataset, GraphType, LegendPosition, Paragraph, Sparkline},
};
...
impl Dashboard {
    ...
    pub fn push_sample(&mut self, t: f64, s: Sample) {
        ...
        while self.history.front().is_some_and(|(t0, _)| *t0 < t - WINDOW_S - 1.0) {
    ...
    pub fn tick(&mut self, ms: u32) {
        ...
        self.pulses.retain(|(_, at)| now.saturating_sub(*at) < PULSE_MS);
    }
...
pub fn dash_areas(area: Rect) -> DashAreas {
    ...
    DashAreas { header, tabs, tiles, cpu, net, cores, memdisk, logs, footer }
}
...
pub fn rate(bps: f64) -> String {
    ...
    if v < 10.0 && i > 0 { format!("{v:.1}{}", UNITS[i]) } else { format!("{v:.0}{}", UNITS[i]) }
}
...
pub fn severity_style(th: &Theme, s: Severity) -> Style {
    ...
    match s {
        ...
        Severity::Warning => Style::new().fg(c.warning_foreground).add_modifier(Modifier::BOLD),
        Severity::Error => Style::new().fg(c.destructive).add_modifier(Modifier::BOLD),
        Severity::Critical => Style::new().bg(c.destructive).fg(c.destructive_foreground).add_modifier(Modifier::BOLD),
    }
}
...
pub fn draw(frame: &mut Frame, d: &Dashboard, v: &View) {
    ...
    frame.render_widget(
        Paragraph::new(Line::from(vec![
            Span::styled(th.label("kp-themes"), Style::new().fg(c.primary).add_modifier(Modifier::BOLD)),
            Span::styled(format!("  {}", v.header), Style::new().fg(c.muted_foreground)),
        ])),
        a.header,
    );
    ...
    let keys =
        " s screen · t theme · p pause · f filter · ↑/↓ PgUp/PgDn scroll · End follow · m motion · r replay · q quit";
    frame.render_widget(Paragraph::new(keys).style(Style::new().bg(c.secondary).fg(c.secondary_foreground)), a.footer);
}
...
fn draw_tiles(frame: &mut Frame, d: &Dashboard, v: &View, a: &DashAreas) {
    ...
    let lines: [(&str, Vec<Span>); 6] = [
        (
            ...
            s.map(|s| {
                let st =
                    pulsed(Metric::Cpu, Style::new().fg(hot_or(Metric::Cpu, c.chart_1)).add_modifier(Modifier::BOLD));
        ...
        (
            ...
            s.map(|s| {
                let st = pulsed(
                    Metric::Memory,
                    Style::new().fg(hot_or(Metric::Memory, c.chart_2)).add_modifier(Modifier::BOLD),
                );
                vec![
                    Span::styled(format!(" {:.0} % ", s.mem_used_pct), st),
                    Span::styled(format!("{:.1}G", s.mem_used_bytes as f64 / (1u64 << 30) as f64), muted),
                ]
            })
            .unwrap_or_else(waiting),
        ),
        (
            "Load",
            s.map(|s| {
                let st = pulsed(Metric::Load, Style::new().fg(hot_or(Metric::Load, c.card_foreground)));
                vec![Span::styled(format!(" {:.2} {:.2} {:.2} ", s.load[0], s.load[1], s.load[2]), st)]
            })
            .unwrap_or_else(waiting),
        ),
        (
        ...
        (
            ...
            s.map(|s| {
                vec![
                    Span::styled("r ", muted),
                    Span::styled(format!("{} ", rate(s.disk_read_bps)), Style::new().fg(c.chart_5)),
                    Span::styled("w ", muted),
        ...
        (
            ...
            s.map(|_| {
                ...
                let own = d.history.iter().map(|(_, s)| s.self_cpu_pct).sum::<f64>() / d.history.len() as f64;
                vec![
    ...
    for (i, ((title, spans), area)) in lines.into_iter().zip(a.tiles).enumerate() {
        ...
        frame.render_widget(Paragraph::new(Line::from(spans)).alignment(Alignment::Center), inner);
    }
}

fn axis_labels(th: &Theme, labels: [String; 3]) -> Vec<Line<'static>> {
    labels.into_iter().map(|l| Line::from(Span::styled(l, Style::new().fg(th.c.muted_foreground)))).collect()
}

fn time_axis(th: &Theme) -> Axis<'static> {
    Axis::default()
        .bounds([0.0, WINDOW_S])
        .labels(axis_labels(th, ["-60s".into(), "-30s".into(), "now".into()]))
...
fn series(d: &Dashboard, f: impl Fn(&Sample) -> f64) -> Vec<(f64, f64)> {
    d.history.iter().map(|(t, s)| (t - d.now_s + WINDOW_S, f(s))).filter(|(x, _)| *x >= 0.0).collect()
}
...
fn draw_cpu(frame: &mut Frame, d: &Dashboard, v: &View, area: Rect) {
    ...
    let y = Axis::default()
        ...
        .style(Style::new().fg(th.c.border_strong));
    frame.render_widget(legend_chart(th, datasets, y), inner.inner(Margin::new(1, 0)));
}
...
fn draw_net(frame: &mut Frame, d: &Dashboard, v: &View, area: Rect) {
    ...
    let y = Axis::default()
        .bounds([0.0, top])
        .labels(axis_labels(th, ["0".into(), format!("{}/s", rate(top / 2.0)), format!("{}/s", rate(top))]))
        .style(Style::new().fg(th.c.border_strong));
    frame.render_widget(legend_chart(th, datasets, y), inner.inner(Margin::new(1, 0)));
}
...
fn draw_cores(frame: &mut Frame, d: &Dashboard, v: &View, area: Rect) {
    ...
    let bars: Vec<Bar> = cores
        ...
        .map(|&pct| {
            ...
            Bar::default().value(pct.round() as u64).text_value(String::new()).style(Style::new().fg(fg))
        })
        .collect();
...
fn draw_memdisk(frame: &mut Frame, d: &Dashboard, v: &View, area: Rect) {
    ...
    let inner = inner(&p, area).inner(Margin::new(1, 0));
    frame.render_widget(p, area);
    let [mem_line, mem_spark, disk_line, disk_spark] =
        Layout::vertical([Constraint::Length(1), Constraint::Fill(1), Constraint::Length(1), Constraint::Fill(1)])
            .areas(inner);
    ...
    if let Some(s) = d.latest() {
        frame.render_widget(
            Paragraph::new(Line::from(vec![
                ...
                Span::styled(format!("{:.1} %", s.mem_used_pct), Style::new().fg(c.chart_2)),
                Span::styled(
                    format!(
                        " · {:.1} of {:.1} GiB",
        ...
        frame.render_widget(
            Paragraph::new(Line::from(vec![
                ...
                Span::styled(format!("{}/s", rate(s.disk_read_bps)), Style::new().fg(c.chart_5)),
                Span::styled(" · write ", muted),
                Span::styled(format!("{}/s", rate(s.disk_write_bps)), Style::new().fg(c.chart_5)),
            ])),
            disk_line,
        );
    }
    ...
    frame.render_widget(
        Sparkline::default().data(&mem).max(1000).style(Style::new().fg(c.chart_2).bg(c.card)),
        mem_spark,
    );
    let [r, w] = Layout::horizontal([Constraint::Percentage(50); 2]).spacing(1).areas(disk_spark);
    ...
    for (data, rect) in [(reads, r), (writes, w)] {
        frame.render_widget(Sparkline::default().data(&data).style(Style::new().fg(c.chart_5).bg(c.card)), rect);
    }
}
...
fn draw_logs(frame: &mut Frame, d: &Dashboard, v: &View, area: Rect) {
    ...
    let l = &d.logs;
    let filter =
        if l.filter == Severity::Debug { "all levels".to_string() } else { format!("{} and up", l.filter.name()) };
    let state = if l.paused() { format!("paused, {} new", l.unseen()) } else { "following".to_string() };
    let status = Line::from(vec![
        Span::styled(format!(" {filter} · "), muted),
        Span::styled(
            state,
            if l.paused() { Style::new().fg(c.warning_foreground).add_modifier(Modifier::BOLD) } else { muted },
        ),
        Span::styled(" ", muted),
    ]);
    let title = if d.feed_live { "Journal · live · read-only" } else { "Log stream · synthetic, not real" };
    let p = panel(v, title, 10).status(status);
    let inner = inner(&p, area).inner(Margin::new(1, 0));
    frame.render_widget(p, area);
    if !d.feed_live && !d.feed_label.is_empty() && l.is_empty() {
        frame.render_widget(Paragraph::new(Span::styled(d.feed_label.clone(), muted)), inner);
        return;
    }

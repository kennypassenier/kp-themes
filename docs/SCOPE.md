...
**scope-136 · A block carries the modules its own markup asks for.**
2026-09-20, the queue form, Kenny's answer **De moduleregel splitsen**,
taken the same evening version 7 landed. Version 7 stopped a block from
being hashed over families its markup never names; the module side was
still whole, so a change to `js/sidenav.js` brought back all eighteen
blocks carrying the navigation component where five hold a side
navigation. The loader already answers the question exactly: `js/auto.js`
holds a table of sixteen modules with the selector each one acts through,
and attaches a module to a root when that root matches. The recipe now
reads that same table — `catalogue/code-version.json` gains a `modules`
map of file to digest and selector, generated by importing `NEEDS` rather
than restating it — and matches it against the block's markup as written,
so a script adding a class while Kenny watches cannot move a hash. What
the loader does not attach by selector (the helpers, the effects, the
React components) stays in the component's own digest. `HASH_VERSION` is
8 and `carry` brought 3045 approvals onto it in 234.1 s; nothing was

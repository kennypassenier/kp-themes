...
| TH72 | Theme `mono` — greyscale, light | Desired | status plates as a lightness ladder + outline/dash; chart pattern fills; needs TH86 **Removed 2026-09-11 (`scope-11`); the promise stands for the set as it was, and is not measurable against the package as it ships.** |
...
| TH86 | Pattern fills for status plates and chart colours | Desired | `--chart-pattern-1..5` and outline/dash badge variants, as tokens per DI9. Test bar: the seven badges of `high-contrast` are pairwise distinguishable with hue removed. **Amended 2026-09-12 (Phase 7): the bar named `mono`, removed at `scope-11`, which made it unreachable — a bar that can never be met can never fail either. `high-contrast` is the greyscale-adjacent theme the package still ships.** |
...
| TH101 | The five documentation gates: coverage, truth, readable ink, one source, layout | Essential | Each has been red once on an injected fault — a component without a page, a prop in the table that is not in the source and a prop in the source that is not in the table, a code ink under 4.5:1 on the surface it sits on [fix-58], a snippet that differs from the example rendered beside it, and a page that scrolls sideways. The drills are recorded in the gate's own comments and in `gates/check-site-ink.test.mjs` |
...
| feat-nav-2 | A side navigation that can be hidden, defaulting to an overlay | Onmisbaar | delivered 2026-09-11 and **withdrawn the same day**, superseded by `feat-nav-3`. It gave the layout class `.kp-sidebar` a toggle, and a layout carries no colour, so no theme ever answered it — which is what made the result look unfinished when Kenny first saw it. The number that decided it: of the six pages using `.kp-sidebar`, exactly one used the hiding, and that one was the documentation page describing it. Removed with its ten tests, its fixture, its module, six knobs and its section of the guide |
| feat-nav-3 | The side navigation as a component of its own, to the feature list Kenny named (MDBootstrap's sidenav) | Onmisbaar | one test per option that carries behaviour — the three modes, the rail, the categories, the edge, the focus — each measured on what the browser paints and each drilled red. Kenny, 2026-09-11: *"ik wil een 'coolere' sidenav die compleet gestyled is"*, after the measurement that 0 of 25 registers mentioned the layout sidebar — **delivered 2026-09-11**, eight tests, four drill passes |
| feat-page-1 | A back-to-top control | Onmisbaar | absent at the top, present after scrolling, and it moves the focus and not only the view — **delivered 2026-09-11**, two tests, both drilled red. The focus half found a defect while being written: focusing the main landmark scrolled straight back down to it on a page with anything tall above the content, so the control undid its own journey. It moves the focus without scrolling now |
| feat-media-1 | A component for an image in the hero | Onmisbaar | the space is there before the image loads, measured on the painted box — **delivered 2026-09-11**, three tests, one drilled red. Measured on an EMPTY frame rather than a slow one: no image at all is the state every page is in for its first moments, and it needs no network to reproduce |
| feat-count-1 | Numbers that count up | Gewenst | at the reduced-motion setting the final number is there immediately, measured on what is painted. **Built 2026-09-12 (Phase 7, `counters`): `data-kp-count` in `js/effects.js`, seven tests in `tests/count.spec.mjs`. The bar is met by a recorder armed before the module exists — reading the text afterwards proves nothing, because the final number is what the HTML says.** |

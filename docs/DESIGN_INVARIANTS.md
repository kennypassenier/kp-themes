...
| DI3 states carry their text | pass | pass | pass | pass | pass | pass | pass |
| DI4 opposed status plates distinguishable | pass | pass | pass | pass | pass | pass | pass |
| DI5 flash threshold | n/a | n/a | n/a | pass | n/a | n/a | n/a |
| DI6 declares colour-scheme and layers rise | pass | pass | pass | pass | pass | pass | pass |
| DI7 reduced motion honoured | pass | pass | pass | pass | pass | pass | pass |
| DI2 | not gated | not gated | not gated | not gated | not gated | not gated | not gated |
| DI9 | not gated | not gated | not gated | not gated | not gated | not gated | not gated |

- **DI2 is not gated:** the two-channel focus ring is a CSS rule, not a token — L4 builds it and the gate that reads it.
- **DI9 is not gated:** layer discipline is a review question about the generated stylesheet, not yet a check.
...
**What is left, and where it belongs.** Two rows read `not gated`, and
that is the honest word for them. DI2's two-channel focus ring is a CSS
rule and a component behaviour rather than a token, so it lands with the
element-level work at L4, together with the check that reads it; the
tokens it needs (`--foreground`, `--background`) already exist and already
sit far enough apart. DI9 is a review question about the generated
stylesheet — whether theme colour stays in the token layer instead of
leaking into element rules — and nothing reads it yet.

L3's exit criterion was "the compliance table reads pass everywhere". It
cannot: two of its rows are not L3's material, and a table that printed
`pass` for an invariant nobody measures would be worth less than one that
admits the gap. Queued as L3-EXIT for the milestone gate.

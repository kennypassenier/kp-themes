...
function yaml(title, rules) {
    ...
    out += `# ${title}\n`;
    out += '# windows/apply.ps1 writes this into Windhawk for you; to do it by hand, paste it into\n';
...
function windhawkReg(mods, label) {
    ...
    const lines = [
        ...
        '',
        `; GENERATED by kp-themes ${VERSION} (gates/generate-windows.mjs) - ${label}. Do not edit.`,
...
function windhawkTranslucent(d) {
    ...
    const text = `# GENERATED by kp-themes ${VERSION} (gates/generate-windows.mjs). Do not edit.
# Translucent Windows 1.8 — https://windhawk.net/mods/translucent-windows
# windows/apply.ps1 writes this into Windhawk for you; to do it by hand, paste it into
...
export function render() {
    ...
    out.set('windhawk-accent/file-explorer-styler.yaml', windhawkExplorer(null).text);
    out.set('windhawk-accent/taskbar-styler.yaml', windhawkTaskbar(null).text);
    out.set('windhawk-accent/start-menu-styler.yaml', windhawkStart(null).text);
    out.set('windhawk-accent/notification-center-styler.yaml', windhawkNotificationCentre(null).text);
    out.set('windhawk-accent/translucent-windows.yaml', windhawkTranslucent(null).text);
    out.set('windhawk-accent/windhawk.reg', windhawkReg(windhawkMods(null), 'accent-following'));
    // VS Code is not part of a switch: its themes are installed once, and which
    // one is active stays the user's (Sundial, or Ctrl+K Ctrl+T).
    out.set(
        'launchers/Install VS Code themes.cmd',
        [
            '@echo off',
            `rem GENERATED by kp-themes ${VERSION} (gates/generate-windows.mjs). Double-click: all KP themes in VS Code; the active theme is left alone.`,
            'powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0..\\windows\\install-vscode.ps1" -Pause',
            '',
        ].join('\r\n'),
    );
    // One file per theme to double-click: the whole desktop in that theme.
    for (const d of all) {
        out.set(
            `launchers/${d.label}.cmd`,
            [
                '@echo off',
                `rem GENERATED by kp-themes ${VERSION} (gates/generate-windows.mjs). Double-click: the whole desktop in KP ${d.label}.`,
                `powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0..\\windows\\apply.ps1" -Theme ${d.name} -Pause`,
                '',
            ].join('\r\n'),
        );
    }
...
function main() {
    ...
    if (process.argv.includes('--check')) {
        ...
        const shared = new Set(['templates', 'wsl', 'windhawk-accent', 'fonts', 'launchers']);

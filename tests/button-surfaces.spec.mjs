...
for (const [channel, url] of CHANNELS) {
    test(
        `the two surfaces cost the control nothing, ${channel} [scope-16]`,
        { tag: ['@component:button', '@component:examples'] },
        async ({ page }) => {
            await page.goto(url);
    ...
    test(
        `a theme that paints neither sees nothing, ${channel} [scope-16]`,
        { tag: ['@component:button', '@component:examples'] },
        async ({ page }) => {
            await page.goto(url);
            const button = page.locator('.kp-button').first();
...
test(
    'the readout is invisible until a theme asks, and never announced [scope-16, KT5]',
    { tag: ['@component:button', '@component:examples'] },
    async ({ page }) => {
        ...
        const heights = await page.evaluate(() =>
            [...document.querySelectorAll('.kp-button')].map((b) => Math.round(b.getBoundingClientRect().height)),
        );
...
test(
    'the two themes that style the readout show it on their own page [readout-words]',
    { tag: ['@component:button', '@component:examples'] },
    async ({ page }) => {

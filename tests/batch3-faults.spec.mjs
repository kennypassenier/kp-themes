...
test(
    'a nav dropdown declared open with data-kp-nav-menu-open is painted open in every theme, and closes when the declaration goes [gap-12]',
    { tag: ['@component:navigation', '@component:catalogue'] },
    async ({ page }) => {
...
test(
    'the selected tab is scrolled into view when it changes from outside [gap-12]',
    { tag: ['@component:navigation', '@component:catalogue'] },
    async ({ page }) => {
...
test(
    'a horizontal split stacks its panes with a horizontal separator in every theme, and the arrows move it [gap-12]',
    { tag: ['@component:structure', '@component:catalogue'] },
    async ({ page }) => {
        ...
        await sweep(page, () => {
            ...
            if (bar.width < whole.width * 0.9)
                faults.push(`the separator is ${Math.round(bar.width)}px wide in a ${Math.round(whole.width)}px split`);
...
test(
    'a row being dragged is visibly marked in every theme, and the mark goes on release [gap-12]',
...
test(
    'the wizard says why it refuses Next when it is not inside a form, and the message goes once the field is filled [gap-12]',
    { tag: ['@component:structure', '@component:catalogue'] },
    async ({ page }) => {
...
test(
    'data-kp-sidenav-slim-toggle collapses and expands the slim rail it controls, says which, and fires the slim event [gap-12]',
    { tag: ['@component:navigation', '@component:catalogue'] },
    async ({ page }) => {
...
test(
    'data-kp-palette-open opens the command palette through its own open, so the open event fires and the list is filtered [gap-12]',

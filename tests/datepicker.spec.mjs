...
const PICKER = '[data-test="plain-date"]';

test('typing a date is enough — the calendar never has to open [TH43]', async ({ page }) => {
    await page.goto(URL);
    const input = page.locator('[data-test="plain-date-input"]');
    ...
    await expect(page.locator('[data-test="plain-date-panel"]')).toBeHidden();
});
...
test('exactly one day is in the tab order [TH43]', async ({ page }) => {
    await page.goto(URL);
    await page.locator('[data-test="plain-date-input"]').fill('4-9-2026');
    await page.locator('[data-test="plain-date-open"]').click();
    const panel = page.locator('[data-test="plain-date-panel"]');
...
test('the arrows cross the month boundary [TH43]', async ({ page }) => {
    await page.goto(URL);
    await page.locator('[data-test="plain-date-input"]').fill('01-09-2026');
    await page.locator('[data-test="plain-date-open"]').click();
    const panel = page.locator('[data-test="plain-date-panel"]');
...
test('every day says which month it is in [TH43]', async ({ page }) => {
    await page.goto(URL);
    await page.locator('[data-test="plain-date-input"]').fill('04-09-2026');
    await page.locator('[data-test="plain-date-open"]').click();
    // "4" alone tells a screen reader nothing about which month it is in.
    await expect(page.locator('[data-kp-day="2026-09-04"]')).toHaveAttribute('aria-label', '4 september 2026');
});

test('Escape closes the grid and returns focus [TH43]', async ({ page }) => {
    await page.goto(URL);
    await page.locator('[data-test="plain-date-input"]').fill('04-09-2026');
    const open = page.locator('[data-test="plain-date-open"]');
    await open.click();
    await page.locator('[data-kp-day="2026-09-04"]').press('Escape');
    await expect(page.locator('[data-test="plain-date-panel"]')).toBeHidden();

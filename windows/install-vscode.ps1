...
.DESCRIPTION
    ...
    Builds a .vsix from ..\vscode\*.json and installs it with VS Code's own CLI.
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File .\windows\install-vscode.ps1
#>
[CmdletBinding()]
param(
    [switch]$Force,
...
elseif (-not (Test-Path $vsSource)) { Say 'vscode' "no vscode\ folder next to windows\ ($vsSource), skipped" 'DarkGray' }
else {
